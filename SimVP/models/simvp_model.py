...
class SimVP_Model_no_skip_no_upsampling(nn.Module):
    r"""SimVP Model
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 mlp_ratio=8., drop=0.0, drop_path=0.0, spatio_kernel_enc=3,
                 spatio_kernel_dec=3, act_inplace=True, **kwargs):
        super(SimVP_Model_no_skip_no_upsampling, self).__init__()
        T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        self.enc = Encoder_no_skip(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.dec = Decoder_no_skip_no_upsampling(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace)
...
class SimVP_Model_no_skip_sst_bath(nn.Module):
    r"""SimVP Model
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 mlp_ratio=8., drop=0.0, drop_path=0.0, spatio_kernel_enc=3,
                 spatio_kernel_dec=3, act_inplace=True, **kwargs):
        super(SimVP_Model_no_skip_sst_bath, self).__init__()
        T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        self.enc1 = Encoder_no_skip(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc2 = Encoder_no_skip(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc3 = Encoder_no_skip(C, 4, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.dec = Decoder_no_skip(2*hid_S+4, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        model_type = 'gsta' if model_type is None else model_type.lower()
        if model_type == 'incepu':
            self.hid = MidIncepNet(T*(hid_S*2+4), hid_T, N_T)
        else:
            self.hid = MidMetaNet(T*(hid_S*2+4), hid_T, N_T,
                input_resolution=(H, W), model_type=model_type,
                mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path)
    ...
    def forward(self, x_raw, **kwargs):
        ...
        embed1 = self.enc1(x[:,0,:,:].reshape(B*T,1,H,W))
        embed2 = self.enc2(x[:,1,:,:].reshape(B*T,1,H,W))
        embed3 = self.enc3(x[:,2,:,:].reshape(B*T,1,H,W))
...
class Encoder_no_skip(nn.Module):
    ...
    def __init__(self, C_in, C_hid, N_S, spatio_kernel, act_inplace=True):
        ...
        self.enc = nn.Sequential(
            ...
            *[ConvSC(C_hid, C_hid, spatio_kernel, downsampling=s,
                     act_inplace=act_inplace) for s in samplings[1:]]
        ) #just applies a lot of convolutions in order, outputting something of size C_hid  
...
class Decoder_no_skip(nn.Module):
    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True):
        samplings = sampling_generator(N_S, reverse=True) #returns [true,false,true,false] when N_s=4 
        ...
        self.dec = nn.Sequential(
            *[ConvSC(C_hid, C_hid, spatio_kernel, upsampling=s,
              ...
              ConvSC(C_hid, C_hid, spatio_kernel, upsampling=samplings[-1],
                     act_inplace=act_inplace)
        ) #applies repeated convolutions
        self.readout = nn.Conv2d(C_hid, C_out, 1) #inputs 2*16+4 input channels, outputs 1 output channel, kernel size is 1
    ...
    def forward(self, hid):
        ...
        Y = self.readout(Y)
        return Y
class Decoder_no_skip_no_upsampling(nn.Module):
    """3D Decoder for SimVP"""

    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True):
        samplings = sampling_generator(N_S, reverse=True) #returns [true,false,true,false] when N_s=4 
        super(Decoder_no_skip_no_upsampling, self).__init__()
        self.dec = nn.Sequential(
            *[ConvSC(C_hid, C_hid, spatio_kernel, upsampling=False,
                     act_inplace=act_inplace) for s in samplings[:-1]], 
              ConvSC(C_hid, C_hid, spatio_kernel, upsampling=False,
                     act_inplace=act_inplace)
        ) #applies repeated convolutions
